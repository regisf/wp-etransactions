...
if (!class_exists('ProductsDb')) {
    class ProductsDb
    {
        ...
        {
            return $this->db->update($this->db_name,
                ['active' => $active],
        ...
        {
            return $this->db->get_var('SELECT COUNT(*) FROM `'. $this->db_name . '` WHERE `active`=true');
        }

        public function get_inactives_count()
        {
            return $this->db->get_var('SELECT COUNT(*) FROM `'. $this->db_name . '` WHERE `active`=false');
        }

        public function get_all_count()
        {
            return $this->db->get_var('SELECT COUNT(*) FROM `'. $this->db_name . '`');
        }

        public function getById($id)
        {
            $results = $this->db->get_results('SELECT * FROM `'. $this->db_name . '` WHERE `product_id`=' . $id);
        ...
        {
            return $this->db->insert($this->db_name, ['name' => $name, 'price' => $price, 'active' => $active]);
        }

        public function deleteById($product_id)
        {
            return $this->db->delete($this->db_name, ['product_id' => $product_id], ['product_id' => $product_id]);
        }
